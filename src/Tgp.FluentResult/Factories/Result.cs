...
namespace Tgp.FluentResult
{
    public partial class Result
    {
        ...
        public static Result Success(HttpStatusCode statusCode = HttpStatusCode.OK)
            => new Result(new MetaHit(null), statusCode);

        /// <summary>
        /// Retorno de sucesso com uma mensagem para o usuário
        /// </summary>
        /// <param name="message">Mensagem amigável para o usuário</param>
        /// <param name="statusCode"><see cref="HttpStatusCode"/></param>
        /// <returns><see cref="Result"/></returns>
        public static Result Success(string message, HttpStatusCode statusCode = HttpStatusCode.OK)
            => new Result(new MetaHit(message), statusCode);
        ...
        /// <param name="statusCode"><see cref="HttpStatusCode"/></param>
        /// <returns><see cref="Result{TResponse}"/></returns>
        public static Result<TResponse> Success<TResponse>(TResponse response, string message, HttpStatusCode statusCode = HttpStatusCode.OK)
            where TResponse : class
            => Result<TResponse>.Success(response, message, statusCode);
    ...
    public partial class Result<TResponse>
    {
        ...
        public static Result<TResponse> Success(TResponse response)
            => new Result<TResponse>(response, new MetaHit(null), HttpStatusCode.OK);
        ...
        /// <param name="statusCode"><see cref="HttpStatusCode"/></param>
        /// <returns><see cref="Result{TResponse}"/></returns>
        public static Result<TResponse> Success(TResponse response, string message, HttpStatusCode statusCode = HttpStatusCode.OK)
            => new Result<TResponse>(response, new MetaHit(message), statusCode);
        ...
        public static async Task<Result<TResponse>> Query(Task<TResponse> queryFunction, string errorMsg, [CallerMemberName] string @callerMethodName = null)
        {
            try
            {
                ...
                {
                    return new Result<TResponse>(result, new MetaHit("Nada foi encontrado."), HttpStatusCode.NotFound);
                }

                return new Result<TResponse>(result, new MetaHit("Dados recuperados com sucesso."), HttpStatusCode.OK);
            }
            catch (Exception ex)
            {
